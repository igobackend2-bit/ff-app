...
interface CartState {
  ...
  // User isolation — call on login to clear cart if a different user was stored
...
export const useCartStore = create<CartState>()(
  persist(
    (set, get) => ({
      ...
      initForUser: (uid) => {
        if (get().userId && get().userId !== uid) {
          // Different user — wipe the previous user's cart
    ...
    }),
    {
      ...
      // Only persist items + userId — not UI state
