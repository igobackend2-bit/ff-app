...
export function OtpLoginForm({ onSuccess }: OtpLoginFormProps = {}) {
  ...
  const sendOtp = useCallback(async (
    ...
    const data = await res.json() as { error?: string; message?: string; emailSent?: boolean };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Isolate cart, wishlist & address to this user (clears if a different user was logged in)
