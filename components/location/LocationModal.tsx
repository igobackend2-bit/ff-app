...
export function LocationModal() {
  ...
  const { address, city, setLocation, detectLocation, isLocating, locationError } = useLocationStore();
  ...
  const handleDetectLocation = useCallback(async () => {
    ...
    // Only close if location was actually detected (no error)
    const state = useLocationStore.getState();
    if (!state.locationError && state.lat !== null) {
      closeLocationModal();
    }
